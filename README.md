...
基于官方 v0.9.1 的本地主线（fork 自 agegr/pi-web），本仓库在官方版本之外新增/增强的功能：

### 侧边栏与会话
- **会话文件树**：侧边栏会话按项目文件夹以文件树展示，折叠状态跨刷新持久化（`36533e5`、`93e0fa1`）
- **浏览器式会话标签页**：多个会话像浏览器标签页一样在顶栏切换（`a9a6fba`）
- **三段式顶栏布局**：顶栏重构为三个分区，点击外部或按 Esc 收起面板；默认折叠的系统提示词组成面板（`12de342`、`0a5c22f`、`96bcd1c`、`a687c36`）

### 用量与余额
- **用量统计面板**：缓存命中率卡片、按供应商的余额追踪、上下文用量 used/total 紧凑展示（`1dd312e`、`814599e`、`b220298`、`83a863f`、`492e3f7`）
- **消息级余额快照**：每条 assistant 消息显示完成时刻的余额快照与真实花费、缓存命中率（4 位小数）；每次运行后自动刷新余额（`173216e`、`dabc868`、`a12818f`、`cd6b6b5`）

### 文件与文件树
- **文件浏览器移入右侧面板**作为固定标签；操作按钮收进文件浏览器工具栏；修复 Windows CRLF 与 node-pty 测试兼容、样式水合不一致（`10db5f8`、`bba4616`、`3435e21`、`f475369`）
- **文件查看器内联编辑**：直接在文件查看器里编辑并保存文件；消息级缓存命中率（`45bd97f`）
- 拖放/选择的文件解析为绝对路径（`80e6dd5`）

### 消息展示
- 供应商与逐消息思考等级显示在 assistant 消息头；带 think 标签的推理拆分为思考块（`1b8296c`、`f825f66`、`a426d90`）
- **分层系统提示词组成**：按插件归因、显示 token 估算（`801d625`、`09c3056`）

### 代理与配置
- **供应商级 HTTP 代理路由**：可为单个被墙的供应商单独配置代理（`a506306`）

### 稳定性与体验
- 编辑器草稿跨会话持久化 + 上下文仪表（`79096a7`）
- Web 主题系统、UX 打磨与准确工具耗时；合并上游 v0.9.0 并适配（`33952d2`、`3e2c3f3`、`6b720de`）

### 文档与工具
- README 中英双语（默认中文）；AGENTS.md 规则与重文档拆分（`b04bcfa`、`cae5156`）
- 缓存命中率分析脚本（`scripts/`，`e977cc5`、`70043a0`、`185525b`）
