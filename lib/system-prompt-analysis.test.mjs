...
test("segments a full default system prompt into labeled sections", () => {
  ...
  assert.ok(keys.includes("context"), "has context group");
  assert.ok(keys.includes("skills"), "has skills");
  assert.ok(keys.includes("cwd"), "has cwd");

  const context = analysis.sections.find((s) => s.key === "context");
  assert.ok(context?.children?.some((c) => c.key === "context:C:/proj/AGENTS.md"), "first context file is a child");
  assert.ok(context.children.some((c) => c.key === "context:C:/proj/docs/EXTRA.md"), "second context file is a child");

  const skills = analysis.sections.find((s) => s.key === "skills");
  assert.ok(skills?.children?.some((c) => c.label === "demo"), "skill entry is a child");
...
test("attributes guideline bullets to plugins via tool hints", () => {
  const prompt = BASE + CONTEXT_BLOCK + SKILLS_BLOCK + CWD_TAIL;
  const tools = [
    {
      name: "todo",
    ...
    {
      name: "replace",
      source: "pi-hashline-edit-pro",
      promptGuidelines: ["Use bash for file operations like ls, rg, find"],
    },
  ];
  const analysis = analyzeSystemPrompt(prompt, tools);
  const guidelines = analysis.sections.find((s) => s.key === "guidelines");
  assert.ok(guidelines?.children);

  const byKey = new Map(guidelines.children.map((c) => [c.key, c]));
  const todo = byKey.get("ext:rpiv-todo");
  assert.ok(todo, "rpiv-todo group exists");
  assert.equal(todo.label, "rpiv-todo");
  assert.equal(todo.detail, "todo");
  const expectedChars = ["Be concise in your responses", "Show file paths clearly when working with files"]
    .reduce((acc, g) => acc + g.length + "- ".length + 1, 0);
  assert.equal(todo.chars, expectedChars);

  const hashline = byKey.get("ext:pi-hashline-edit-pro");
  assert.ok(hashline, "hashline group exists");
  assert.equal(hashline.detail, "replace");

  const core = byKey.get("pi-core");
  assert.ok(core, "unmatched bullets fall into pi core");
  assert.ok(core.chars > 0);

  const childSum = guidelines.children.reduce((acc, c) => acc + c.chars, 0);
  assert.equal(childSum, guidelines.chars, "guideline children fully cover the zone");
});

test("tools section lists one child per tool line", () => {
  const prompt = BASE + CONTEXT_BLOCK + SKILLS_BLOCK + CWD_TAIL;
  const analysis = analyzeSystemPrompt(prompt);
  const tools = analysis.sections.find((s) => s.key === "tools");
  assert.ok(tools?.children);
  // Children are sorted largest first.
  assert.deepEqual(tools.children.map((c) => c.label), ["read", "edit", "bash"]);
  const childSum = tools.children.reduce((acc, c) => acc + c.chars, 0);
  assert.ok(childSum > 0 && childSum <= tools.chars);
});
...
test("token counts follow the chars/4 heuristic per section", () => {
  ...
  const toolsEnd = prompt.indexOf("\n\nIn addition to the tools above, you may have access to other custom tools depending on the project.") + 101;
  const tools = analysis.sections.find((s) => s.key === "tools");
  assert.ok(tools);
  assert.equal(tools.chars, toolsEnd - toolsStart);
  // A parent section reads as the sum of its children (tokens conserved).
  const childSum = tools.children.reduce((acc, c) => acc + c.tokens, 0);
  assert.equal(tools.tokens, childSum);
  // A leaf section is counted exactly with tiktoken.
  const guidelines = analysis.sections.find((s) => s.key === "guidelines");
  assert.ok(guidelines);
  assert.equal(guidelines.tokens, estimateTokensOf(guidelines.text));

  const guidelinesLabel = analysis.sections.find((s) => s.key === "guidelines");
  assert.ok(guidelinesLabel);
  assert.equal(guidelinesLabel.labelKey, "system.section.guidelines");
  const guideStart = prompt.indexOf("\n\nGuidelines:\n");
  const contextStart = prompt.indexOf("<project_context>");
  // The span starts after the leading blank line pair, which belongs to base.
  assert.equal(guidelines.chars, contextStart - (guideStart + 2));

  const contextGroup = analysis.sections.find((s) => s.key === "context");
  const agents = contextGroup?.children?.find((c) => c.key === "context:C:/proj/AGENTS.md");
  assert.ok(agents);
  const agentsBlock = `<project_instructions path="C:/proj/AGENTS.md">\n# Agent notes\nUse pnpm, never npm.\n</project_instructions>`;
  assert.equal(agents.chars, agentsBlock.length);
  assert.equal(agents.label, "C:/proj/AGENTS.md");
...
test("empty prompts produce an empty analysis", () => {
  ...
  assert.deepEqual(analysis, { sections: [], segments: [], totalChars: 0, totalTokens: 0 });
});
