...
test("a project-level value is reported as shadowing the global one", async () => {
  ...
  assert.deepEqual(readEnabledModelsSettings(manager, paths), {
    patterns: ["anthropic/*"],
    scope: "global",
    // Windows keeps %TEMP% under the home directory, so displayPath shortens
    // it to `~`; mirror that normalization in the expectation.
    path: underHome(join(agentDir, "settings.json")),
  });
  ...
  assert.deepEqual(readEnabledModelsSettings(manager, paths), {
    patterns: ["openai/*"],
    scope: "project",
    path: underHome(join(projectDir, ".pi", "settings.json")),
  });
});
